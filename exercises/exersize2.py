...
if data:
    ...
    try:
        ...
        df = pd.read_csv(url_dataset, encoding='latin1', sep=';', error_bad_lines=False, names=['EVA_NR', 'DS100', 'IFOPT', 'NAME', 'Verkehr', 'Laenge', 'Breite', 'Betreiber_Name', 'Betreiber_Nr', 'Status'])
        ...
        if 'Verkehr' in df.columns and 'Laenge' in df.columns and 'Breite' in df.columns:
            ...
            df = df[(df['Laenge'].between(-90, 90, inclusive=True)) & (df['Breite'].between(-90, 90, inclusive=True))]
